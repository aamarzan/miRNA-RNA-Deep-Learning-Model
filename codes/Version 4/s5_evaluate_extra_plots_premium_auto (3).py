...
def figS3_distribution_alignment(y_true,y_pred,out_dir,stdnames,cmap="magma"):
    ...
    # ---- Colorbar placed OUTSIDE the axes in figure coordinates for guaranteed spacing
    pos = ax.get_position()
    gap = 0.06     # horizontal gap to the right of the plot
    cbar_w = 0.02  # colorbar width
    cax = fig.add_axes([pos.x1 + gap, pos.y0, cbar_w, pos.height])
...
def figS9_pr_curve(y_true, y_pred, cfg, out_dir, stdnames, cmap="cubehelix"):
    y_bin, rule = _binarize_ground_truth(y_true, cfg, default_top_frac=0.10)
    pos = int(np.sum(y_bin==1)); neg = int(np.sum(y_bin==0))
    if pos == 0 or neg == 0:
        info = {"available": False, "reason": "Positive or negative count is zero", "rule": rule, "pos": pos, "neg": neg}
