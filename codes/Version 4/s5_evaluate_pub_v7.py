...
# -*- coding: utf-8 -*-
"""
s5_evaluate_pub_v7.py  — final micro-tweaks for figures 3a–S2
Requests implemented:
- 3a, 3b unchanged
- 3c: stats NW, smaller font
- 3d: one stats box NW (Fit+JB)
- 3e: smaller legend font so it never clashes with stats box
- S1: no title; stats box also states “Solid: train | Dashed: val”
- S2: stronger Spectral (alpha=0.85)
"""
...
def load_npz_test(data_path):
    X_test={}
    tfs=[f for f in os.listdir(data_path) if f.startswith("X_test_") and f.endswith(".npz")]
    if not tfs: raise FileNotFoundError(f"No X_test_*.npz in {data_path}")
    for f in sorted(tfs):
...
def fig3a(y_true,y_pred,out_dir,formats,stdnames,jitter=0.001,outline=True):
    ...
    place_stats_box(ax,txt,corner="NE",pad=0.03,fontsize=8.2)
...
def fig3c(y_true,y_pred,out_dir,formats,stdnames):
    ...
    place_stats_box(ax,txt,corner="NW",pad=0.02,fontsize=7.9)  # NW + smaller
...
def fig3d(y_true,y_pred,out_dir,formats,stdnames):
    ...
    txt=f"Fit (R={R:.3f})\nJB p={jb_p:.3g}"   # combined into one box, NW
    place_stats_box(ax,txt,corner="NW",pad=0.03,fontsize=8.2)
...
def fig3e(y_true,y_pred,out_dir,formats,stdnames,jitter=0.001,outline=True):
    ...
    for t in leg.get_texts(): t.set_fontsize(8.0)  # smaller legend font
...
def figS1(history,out_dir,formats,stdnames):
    ...
    # No title; embed legend text inside the stats box
    best_txt=""; 
    if len(va)>0:
...
def figS2(y_true,y_pred,out_dir,formats,stdnames):
    # Real Spectral (stronger alpha)
    cmap=plt.get_cmap("Spectral")
    abs_err=np.abs(y_true-y_pred)
    bins=np.arange(0,1.00001,0.1)
    ...
    smap=mpl.cm.ScalarMappable(cmap=cmap,
                               norm=mpl.colors.Normalize(vmin=mn,vmax=mx)); smap.set_array([])
...
def main():
    ...
    out_dir=args.out if args.out else os.path.join(out_root,f"evalv7_{timestamp}")
