#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# (See inline docstring for usage; built-in defaults set to journal presets.)
...
def bland_altman_variants(y_true, y_pred, out_dir, formats, stdnames, variants, palettes, jitter_scale, outline):
    ...
    loA = mean_diff - 1.96*sdiff if (sdiff := sd_diff) else mean_diff - 1.96*sd_diff
    hiA = mean_diff + 1.96*sdiff if (sdiff := sd_diff) else mean_diff + 1.96*sd_diff
...
def main():
    ...
    # Built-in defaults matching your requested flags
    ...
    ap.add_argument("--palette-list", type=str, default="viridis,plasma,cividis,magma,inferno,turbo,coolwarm,Spectral")
    ap.add_argument("--color-by", type=str, default="density",
                    choices=["density","residual","abs_residual","true","pred","bin_true","bin_pred","bin_residual"])
    ap.add_argument("--size-by", type=str, default="abs_residual",
                    choices=["none","residual","abs_residual"])
    ap.add_argument("--point-size", type=float, nargs=2, default=[6.0, 18.0])
