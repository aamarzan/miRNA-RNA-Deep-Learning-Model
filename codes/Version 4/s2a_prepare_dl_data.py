# 2_deep_learning_data_preparation.py (Final unbiased, group-split, train-only-scaler version)
import os
import pandas as pd
import numpy as np
import json
...
def main():
    ...
    # --- Step 3: Group-wise split by (primary_id || target_id) and fit scaler on TRAIN ONLY ---
    step1_start_time = time.time()
    print("\nStep 1: Creating group-wise split and fitting the scaler on TRAIN only...")

    # Load IDs for grouping (cheap)
    id_cols = ['primary_id', 'target_id']
    try:
        id_df = pd.read_parquet(prepared_dataset_path, columns=id_cols)
    except Exception as e:
        print(f"  - FATAL ERROR: Could not read ID columns for split: {e}")
        exit()

    groups = (id_df['primary_id'].astype(str) + '||' + id_df['target_id'].astype(str)).values
    unique_groups = pd.unique(groups)

    rng = np.random.RandomState(42)
    rng.shuffle(unique_groups)
    cut = int((1.0 - params.get('test_split_ratio', 0.2)) * len(unique_groups))
    train_groups = set(unique_groups[:cut])
    test_groups = set(unique_groups[cut:])

    # Build per-row assignment (train/test)
    sample_assignment = np.where(np.isin(groups, list(train_groups)), 'train', 'test')
    del id_df, groups, unique_groups

    # Fit scaler on TRAIN rows only (no leakage)
    numerical_features = params.get('numerical_features', ['gc_content', 'dg', 'conservation'])
    try:
        full_num = pd.read_parquet(prepared_dataset_path, columns=numerical_features)
    except Exception as e:
    ...
    scaler = MinMaxScaler()
    try:
        scaler.fit(full_num[sample_assignment == 'train'])
    except Exception as e:
        print(f"  - FATAL ERROR: Scaler fit failed: {e}")
        exit()

    joblib.dump(scaler, os.path.join(output_dl_folder, 'minmax_scaler.pkl'))
    print(f"  - Scaler fitted on TRAIN split and saved.")
    del full_num

    step1_time = time.time() - step1_start_time
    print(f"  - Time for Step 1: {step1_time:.2f} seconds")

    # --- Step 2: Prepare Parquet streaming and shapes ---
    step2_start_time = time.time()
    print("\nStep 2: Planning batch streaming and target feature...")
    ...
    max_primary_len, max_target_len, max_competitor_len = (
        pad_params['max_primary_len'],
        pad_params['max_target_len'],
        pad_params['max_competitor_len'],
    )
    ...
    # --- Step 3: Process and Save Datasets in Batches ---
    ...
    def process_chunk(df_chunk, scaler_obj):
        ...
        X = {
            ...
            'primary_structure_input': np.expand_dims(
                pad_sequences(
                    df_chunk['structure_vector'].apply(json.loads).tolist(),
                    maxlen=max_primary_len,
                    padding='post',
                    dtype='float32'
                ),
                axis=-1
    ...
    # Warm-up: inspect shape for dict keys
    first_batch_df = next(parquet_file.iter_batches(batch_size=1)).to_pandas()
    # Ensure required columns exist
    required_cols = [
        'primary_sequence', 'target_sequence', 'competitor_sequence',
        'structure_vector', *numerical_features, target_feature
    ]
    missing = [c for c in required_cols if c not in first_batch_df.columns]
    if missing:
        print(f"  - FATAL ERROR: Missing required columns in dataset: {missing}")
        exit()
    ...
    # Stream and assign per batch using the precomputed sample_assignment
    processed_rows = 0
    for batch in tqdm(parquet_file.iter_batches(batch_size=batch_size), total=total_batches, desc="  Processing batches"):
        df = batch.to_pandas()

        # Sanity: ensure columns present (in case of column pruning elsewhere)
        for c in required_cols:
            if c not in df.columns:
                df[c] = first_batch_df[c].iloc[0] if c in first_batch_df.columns else 0

        start_idx = processed_rows
        end_idx = processed_rows + len(df)
        assignments = sample_assignment[start_idx:end_idx]
    ...
    # --- Step 4: Finalize and Save Arrays ---
    step4_start_time = time.time()
    print("\nStep 4: Concatenating and saving final compressed .npz arrays...")

    # Save training data (apply sqrt transform for targets, matching training/inference pipeline)
    if y_train_batches:
