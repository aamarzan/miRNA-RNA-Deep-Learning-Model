#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# See full docstring inside for usage and defaults.
...
def fig3a_pred_vs_obs(y_true, y_pred, r2, pr, out_dir, formats, stdnames, jitter_scale=0.001, outline=True):
    ...
    ax.set_title(f'Predicted vs Observed (R²={r2:.3f}; r={pr:.3f})')
...
def fig3b_residuals_vs_fitted(y_true, y_pred, out_dir, formats, stdnames, jitter_scale=0.001, outline=True):
    ...
    if HAS_SM and len(xp)>50:
        ...
        ax.plot(xp, low, c='black', lw=1.2, label='LOWESS'); ax.legend()
...
def fig3c_residual_hist(y_true, y_pred, out_dir, formats, stdnames):
    cmap = plt.get_cmap("turbo")
    residuals = y_true - y_pred
    fig = plt.figure(figsize=golden_figsize(4.9)); ax = fig.add_subplot(111)
    counts, bins, patches = ax.hist(residuals, bins=60, edgecolor='none')
    counts = counts.astype(float); cmin, cmax = counts.min(), counts.max()
    ...
    base = os.path.join(out_dir, 'Figure3c_residual_histogram' if stdnames else 'residuals_distribution')
    return save_multi(fig, base, formats)

def fig3d_qq(y_true, y_pred, out_dir, formats, stdnames):
    ...
    base = os.path.join(out_dir, 'Figure3d_qq_plot' if stdnames else 'qq_plot')
    return save_multi(fig, base, formats)
...
def fig3e_bland_altman(y_true, y_pred, out_dir, formats, stdnames, jitter_scale=0.001, outline=True):
    ...
    ax.set_title(f'Bland–Altman (outside LoA: {frac_out:.1f}%)')
    ax.legend()
...
def fig4c_reliability(y_true, y_pred, out_dir, formats, stdnames, n_bins=15):
    ...
    ax.set_title(f'Reliability diagram (ECE={ece:.3f}, MCE={mce:.3f})')
...
def main():
    ...
    out_dir = args.out if args.out else os.path.join(out_root, f"evalv3_{timestamp}")
    ...
    saved = []
    saved += fig3a_pred_vs_obs(y_true, y_pred, r2, pr, out_dir, formats, args.stdnames, jitter_scale=args.jitter, outline=args.outline)
    ...
    saved += fig3c_residual_hist(y_true, y_pred, out_dir, formats, args.stdnames)
    saved += fig3d_qq(y_true, y_pred, out_dir, formats, args.stdnames)
    ...
    manifest = {"meta": meta, "metrics": {"R2": float(r2), "pearson_r": float(pr), "MSE": float(mse), "MAE": float(mae)}, "files": {"figures": saved}}
