...
# -*- coding: utf-8 -*-
"""
s5_evaluate_pub_v6.py
- Stats panels inside axes with collision-avoidance vs legends & colorbars
- 3b LOWESS removed (hard-off)
- S1 training history uses turbo gradient with a colorbar
"""
...
def save_multi(fig, base, formats):
    outs=[]
    for ext in formats:
        p=f"{base}.{ext}"; fig.savefig(p, bbox_inches="tight"); outs.append(p)
...
def choose_corners_by_density(x,y,ax,fw=0.34,fh=0.28):
    x0,x1=ax.get_xlim(); y0,y1=ax.get_ylim(); rx,ry=x1-x0,y1-y0
    try:
        kde=gaussian_kde(np.vstack([x,y]))
        def dens(xa,xb,ya,yb,n=22):
            X,Y=np.meshgrid(np.linspace(xa,xb,n),np.linspace(ya,yb,n))
            Z=kde(np.vstack([X.ravel(),Y.ravel()])); return Z.mean()
    except Exception:
        def dens(*args,**kw): return 0.5
    corners={
        "SW":(x0,x0+fw*rx,y0,y0+fh*ry),
        "SE":(x1-fw*rx,x1,y0,y0+fh*ry),
        "NW":(x0,x0+fw*rx,y1-fh*ry,y1),
        "NE":(x1-fw*rx,x1,y1-fh*ry,y1),
    }
    scores={k:dens(*v) for k,v in corners.items()}
    return sorted(scores.keys(), key=lambda k:scores[k])  # low→high density
...
def fig3a(y_true,y_pred,out_dir,formats,stdnames,jitter=0.001,outline=True):
    ...
    ax.legend(loc="upper left")  # block NW
    ...
    # Avoid NW (legend), prefer least-dense of others
    for c in [c for c in choose_corners_by_density(xx,yy,ax) if c!="NW"]:
        place_stats_box(ax,txt,corner=c,pad=0.03,fontsize=8.2); break
...
def fig3b(y_true,y_pred,out_dir,formats,stdnames,jitter=0.001,outline=True):
    ...
    place_stats_box(ax,txt,corner="NW",pad=0.03,fontsize=8.2)  # avoid right colorbar
...
def fig3d(y_true,y_pred,out_dir,formats,stdnames):
    resid=y_true-y_pred
    try: norm_txt=f"JB p={jarque_bera(resid)[1]:.3g}"
    except Exception: norm_txt="Normality test error"
    ...
    slope,intercept,r=osr; ax.plot(theo,slope*theo+intercept,c="black",lw=1.2,label=f"Fit (R={r:.3f})")
    ax.set_title("Q–Q plot of residuals"); ax.set_xlabel("Theoretical quantiles"); ax.set_ylabel("Ordered residuals")
    ax.legend(loc="upper left")
    place_stats_box(ax,norm_txt,corner="NW",pad=0.03,fontsize=8.2)  # avoid right cbar
...
def fig3e(y_true,y_pred,out_dir,formats,stdnames,jitter=0.001,outline=True):
    ...
    ax.set_title("Bland–Altman (no shading)"); ax.legend(loc="upper right")
...
def figS1(history,out_dir,formats,stdnames):
    ...
    if len(tr)>0 and len(va)>0:
        place_stats_box(ax,f"Train end={tr[-1]:.4f} | Val best={np.nanmin(va):.4f} (ep {best_idx})",corner="NW",pad=0.03,fontsize=8.2)
    smap=mpl.cm.ScalarMappable(cmap=cmap,norm=norm); smap.set_array([]); cbar=plt.colorbar(smap,ax=ax,orientation="vertical"); cbar.set_label("Epoch (training)")
...
def figS2(y_true,y_pred,out_dir,formats,stdnames):
    ...
    for j,patch in enumerate(bp["boxes"]):
        patch.set_facecolor(cmap(0.5 if mx==mn or np.isnan(med[j]) else (med[j]-mn)/(mx-mn))); patch.set_alpha(0.40)
...
def main():
    ...
    out_dir=args.out if args.out else os.path.join(out_root,f"evalv6_{timestamp}")
