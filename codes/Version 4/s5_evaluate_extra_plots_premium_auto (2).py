...
def _roc_manual(y_true_bin, y_score):
...
def figS3_distribution_alignment(y_true,y_pred,out_dir,stdnames,cmap="magma"):
    ...
    # Colorbar moved FARTHER right using axes_grid1
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="4%", pad=0.90)  # much larger pad to avoid any overlap
    m = mpl.cm.ScalarMappable(norm=Normalize(vmin=0, vmax=np.nanmax(ecdf_abs_diff)), cmap=cmap)
    cbar = plt.colorbar(m, cax=cax); cbar.set_label("ECDF |Δ|", rotation=90)
...
def figS6_interval_coverage(y_true,y_pred,out_dir,stdnames,cmap="cividis"):
    ...
    ax.set_title("Prediction Interval Coverage (points colored by |obs−nom|)", y=1.10)  # lifted even higher
...
def figS8_roc_curve(y_true, y_pred, cfg, out_dir, stdnames, cmap="inferno"):
    ...
    fpr, tpr, thresholds, auc_val, best = _roc_manual(y_bin, y_pred)
    if fpr is None:
    ...
    ax.scatter([fpr_best],[tpr_best], s=45, facecolors="none", edgecolors="black", linewidths=1.2, zorder=4, label="Youden-optimal")
...
def main():
    ...
    print("[✓] Premium extra plots (incl. ROC) complete:", out_dir)
