# s4_predict.py (Definitive, Model-Aware Version)
import os
import json
import numpy as np
import pandas as pd
...
def main():
    ...
    if not primary_records or not target_records:
        print("\nAborting: Missing primary sequences to rank or target sequences.")
        return
    ...
    target_processed = process_rna_universal(((target_record.id, target_seq), config, 'target_molecule'))
    if competitor_record:
        competitor_processed = process_rna_universal(((competitor_record.id, str(competitor_record.seq)), config, 'competitor_molecule'))
    else:
    ...
    # <<< CHANGE: Auto-detect padding lengths directly from the loaded model >>>
    ...
    try:
        ...
        print(f"  - WARNING: Could not auto-detect lengths. Falling back to config.json. Error: {e}")
        pad_lengths = (
    ...
    for i, primary_record in enumerate(primary_records):
        primary_processed = process_rna_universal(((primary_record.id, str(primary_record.seq)), config, 'primary_molecule'))
    ...
    if results:
        ...
        pq.write_table(pa.Table.from_pandas(results_df), output_path)
